function [accuracy,mew,sensitivity,specificity,training_time]=non_L_TWSVMf(A,A_test,C1,mew)
tic
C2=C1;
...
K3=zeros(size(A_test,1),x+x1);
               %       (repmat(sqrt(sum(u.^2,2).^2),1,size(v,1))-2*(u*v')+repmat(sqrt(sum(v.^2,2)'.^2),size(u,1),1)));
 ...
 %   py1=abs(u1(1:size(u1,1)-1,:)*xtest0(i,:) + u1(size(u1,1),:))/norm(u1(1:size(u1,1)-1,:));
  %  py2=abs(u2(1:size(u2,1)-1,:).*xtest0(i,:) + u2(size(u2,1),:))/norm(u2(1:size(u2,1)-1,:));
   % preY1=[preY1;py1];
    %preY2=[preY2;py2];
%end    
    preY2=(K3*u2(1:size(u2,1)-1,:)+u2(size(u2,1),:));
    preY1=(K3*u1(1:size(u1,1)-1,:)+u1(size(u1,1),:));
